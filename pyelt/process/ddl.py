...
#####################################
    ...
    def create_or_alter_ref(self, mapping):
        #maak reftable
        ref = self.dwh.ref
        if not ref.is_reflected:
            ref.reflect()
        if mapping.target not in ref:
            params = {}
            params.update(self._get_fixed_params())
            params['fixed_columns_def'] = self.__get_fixed_sat_columns_def()
            params['ref_table_name'] = mapping.ref_type  # ref_type is de table_name
            ...
            sql = """CREATE TABLE IF NOT EXISTS {ref}.{ref_table_name} (
                      ...
                      CONSTRAINT ref_code_unique_{ref_table_name} UNIQUE (_runid, valueset_naam, code, niveau, niveau_type)
                )
                WITH (
                  OIDS=FALSE,
                  autovacuum_enabled=true
                );""".format(**params)
            self.execute(sql, 'create ref tables')

    def __get_fixed_sat_columns_def(self):
        sql = """
            _id serial NOT NULL,
            _runid numeric(8,2) NOT NULL,
            _active boolean DEFAULT TRUE,
