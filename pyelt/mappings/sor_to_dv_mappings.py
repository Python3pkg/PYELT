...
class SorToEntityMapping(BaseTableMapping):
    ...
    def try_append_default_value_mappings(self):
        for sat in self.entity.get_sats().values():
...
class SorToLinkMapping(BaseTableMapping):
    ...
    def get_fk_name(self, entity_cls):
        for prop_name, entity in self.target.__dict__.items():
            if entity == entity_cls or entity == entity_cls.__base__:
                fk = """fk_{}""".format(entity.get_hub_name())
                if prop_name.lower() != entity.__name__.lower():
                    fk = """fk_{}_{}""".format(prop_name.lower(), entity.get_hub_name())
                return fk
    ...
    def map_bk(self, source_bk, target_link_ref: Union[LinkReference, DynamicLinkReference], join: str = '', type: Union[str, 'DvEntity'] = ''):
        ...
        if isinstance(target_link_ref, LinkReference):
            ...
            hub = target_link_ref.entity_cls.get_hub()
    ...
    def map_entity_old(self, entity_cls: DvEntity,  bk: str = '', join: str = '', type: str = ''):
        # is_hybrid = False
        hub_name = entity_cls.get_hub_name()
        hub = entity_cls.get_hub()
...
class SorToRefMapping(BaseTableMapping):
    def __init__(self, source: Union[str, Dict[str, str]], ref_type: str = '', sor: Schema = None, filter: str = '') -> None:
        target = ref_type
        super().__init__(source, target)
    ...
    ### nieuw aangemaakt om extra column te krijgen
    def map_type_field(self, source_type_field: str, source_oid_field: str = "''") -> None:
        target = 'valueset_type_naam'
    ...
    #     self.source_type_field = source_type_field
        ...
        # def map_datum_van_field(self, source_datum_van_field: str)-> None:
        #     target = 'datum_van'
        #     if '.' in source_datum_van_field:
        #         source_datum_van_field = source_datum_van_field.replace(self.sor_table + '.', '')
        #     self.map_field(source_datum_van_field, target)
        #     self.source_datum_van_field = source_datum_van_field
        #
        # def map_datum_tot_field(self, source_datum_tot_field: str)-> None:
        #     target = 'datum_tot'
        #     if '.' in source_datum_tot_field:
        #         source_datum_tot_field = source_datum_tot_field.replace(self.sor_table + '.', '')
        #     self.map_field(source_datum_tot_field, target)
        #     self.source_datum_van_field = source_datum_tot_field
        #
        # def map_status_field(self, source_status_field: str)-> None:
        #     target = 'status'
        #     if '.' in source_status_field:
        #         source_status_field = source_status_field.replace(self.sor_table + '.', '')
        #     self.map_field(source_status_field, target)
        #     self.source_status_field = source_status_field
        #
        # def map_versie_field(self, source_versie_field: str)-> None:
        #     target = 'versie'
        #     if '.' in source_versie_field:
        #        source_versie_field = source_versie_field.replace(self.sor_table + '.', '')
        #     self.map_field(source_versie_field, target)
        #     self.source_versie_field = source_versie_field
        #
        # def map_projecten_field(self, source_projecten_field: str)-> None:
        #     target = 'projecten'
        #     if '.' in source_projecten_field:
        #        source_projecten_field = source_projecten_field.replace(self.sor_table + '.', '')
        #     self.map_field(source_projecten_field, target)
        #     self.source_projecten_field = source_projecten_field
